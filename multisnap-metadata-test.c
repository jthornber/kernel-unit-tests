...
typedef int (*test_fn)(struct multisnap_metadata *tpm);

/*----------------------------------------------------------------*/

static int check_empty_transaction(struct multisnap_metadata *tpm)
{
	return multisnap_metadata_commit(tpm);
}

static int check_insert_1_transaction(struct multisnap_metadata *tpm)
{
	int r;
	...
	for (tb = 0; tb < DATA_DEV_SIZE; tb++) {
		...
		r = multisnap_metadata_insert(tpm, 0, tb, map + tb);
		if (r < 0) {
		...
		r = multisnap_metadata_lookup(tpm, 0, tb, 1, &pb);
		if (r < 0) {
	...
	r = multisnap_metadata_commit(tpm);
	if (r < 0) {
...
static int check_insert_multi_transaction(struct multisnap_metadata *tpm)
{
	int r;
	static block_t map[DATA_DEV_SIZE];
	block_t tb;
...
static int check_accessors(struct multisnap_metadata *tpm)
{
	int r;
	sector_t s;
	block_t b;

	r = multisnap_metadata_get_data_block_size(tpm, 0, &s);
	if (r < 0) {
	...
	r = multisnap_metadata_get_data_dev_size(tpm, 0, &b);
	if (r < 0) {
	...
	r = multisnap_metadata_get_provisioned_blocks(tpm, 0, &b);
	if (r < 0) {
	...
	r = multisnap_metadata_resize_data_dev(tpm, 0, 101);
	if (r < 0) {
...
static int run_test(const char *name, test_fn fn)
{
	...
	tpm = multisnap_metadata_open(bdev,
				      DATA_BLOCK_SIZE,
				      DATA_DEV_SIZE);
	if (!tpm) {
